...
import { DB_HOST, SECRET_DB_HOST } from '$env/static/private'
...
export async function load({ params, cookies }) {
	...
	console.log('import.meta.env.DB_HOST', import.meta.env.DB_HOST)
	// VITE_ENV_TEST VITE_로 시작하면 import.meta.env. 로 사용 가능 (front.svelte에서도 쓸 수 있음)
	// 이 변수는 .env.local 에 적힌 값으로 대체 되었음
	console.log('import.meta.env.VITE_TEST_ENV', import.meta.env.VITE_TEST_ENV)
	// DB_HOST private 서버 사이드 ENV는 위에서 import로 가져와야 함.
	// 이 변수는 .env.local 에 적힌 값으로 대체 안됨 ??? 대체 왜지 ???
	console.log('DB_HOST', DB_HOST)
	console.log('SECRET_DB_HOST', SECRET_DB_HOST)
	// PUBLIC_TEST_ENV public 방식은 sveltekit 에서 지원하는 public key 방식
	console.log('PUBLIC_TEST_ENV', PUBLIC_TEST_ENV)
	// 이건 sveltekit 에서 지원하는 방식이 아닌 VITE에서 지원하는 방식이므로 못씀
	console.log('import.meta.env.PUBLIC_TEST_ENV', import.meta.env.PUBLIC_TEST_ENV)
